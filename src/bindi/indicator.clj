...
(defn- atr*
  ([state quote] (atr* state quote :atr 14))
  ([state quote kw n]
   (evaluate-once
    [state kw quote]
    (let [{{:keys [atr2], pc :c} kw} state
          {:keys [t h l c]} quote
          [atr2 atr]
          (if-not pc
            ;; very first quote
            ...
            (let [tr (- (max h pc) (min l pc))
                  a (smooth-avg atr2 tr n)]
              [a (if (number? a) {:t t, :i a})]))]
      [atr (assoc state kw {:atr2 atr2, :c c})]))))

(defn- pos-di*
  ([state quote] (pos-di* state quote :pos-di 14 atr*))
  ([state quote kw n atr*]
   (evaluate-once
    [state kw quote]
    (let [[{atr :i} state] (atr* state quote)
          {{:keys [avg-dm], ph :h, pl :l} kw} state
          {:keys [t h l]}  quote
          [avg-dm pos-di]
          (if-not ph
            ;; very first quote
            ...
            (let [+dm (- h ph), -dm (- pl l)
                  ...
                  a (smooth-avg avg-dm dm n)
                  di (if (and atr (number? a)) (/ a atr 0.01))]
              [a (if di {:t t, :i di})]))]
      [pos-di (assoc state kw {:avg-dm avg-dm, :h h, :l l})]))))

(defn- neg-di*
  ([state quote] (neg-di* state quote :neg-di 14 atr*))
  ([state quote kw n atr*]
   (evaluate-once
    [state kw quote]
    (let [[{atr :i} state] (atr* state quote)
          ...
          (if-not ph
            ...
            (let [+dm (- h ph), -dm (- pl l)
                  ...
                  di (if (and atr (number? a)) (/ a atr 0.01))]
              [a (if di {:t t, :i di})]))]
      [neg-di (assoc state kw {:avg-dm avg-dm, :h h, :l l})]))))

(defn- adx*
  ([state quote] (adx* state quote :adx 14 pos-di* neg-di*))
  ([state quote kw n pos-di* neg-di*]
   (evaluate-once
    [state kw quote]
    (let [[{+di :i} state] (pos-di* state quote)
          ...
          (if (and +di -di)
            (let [dx (/ (Math/abs (- +di -di)) (+ +di -di) 0.01)
                  a (smooth-avg adx2 dx n)]
              [a (if (number? a) {:t t, :i a})]))]
      [adx (assoc state kw {:adx2 adx2})]))))

(defn- atr-30* [state quote] (atr* state quote :atr-30 30))
(defn- pos-di-30* [state quote] (pos-di* state quote :pos-di-30 30 atr-30*))
(defn- neg-di-30* [state quote] (neg-di* state quote :neg-di-30 30 atr-30*))
(defn- adx-30* [state quote] (adx* state quote :adx-30 30 pos-di-30* neg-di-30*))
